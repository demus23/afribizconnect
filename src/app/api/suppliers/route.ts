...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const orderBy: any = sort === 'name' ? { name: 'asc' } : sort === 'newest' ? { createdAt: 'desc' } : { trustScore: 'desc' }

    const [businesses, total] = await Promise.all([
      prisma.business.findMany({ where, orderBy, skip, take: limit, include: { asSupplier: true } }),
